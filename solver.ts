class Solver {
  ...
  newGuess(g: string, code: string): string {
    ...
    let oc = new Map<string, number>(); // map of letter to number of occurrences
    this.wordlist.forEach(w => {
      for (let i = 0; i < w.length; i++) {
        if (oc.has(w[i])) oc.set(w[i], oc.get(w[i]) + 1);
      }
    });
    let scores: number[] = [];
    this.wordlist.forEach(w => {
    ...
    if(this.wordlist.length == 0) return "No guesses found. Refresh page to try again."
    return lg;
  }
